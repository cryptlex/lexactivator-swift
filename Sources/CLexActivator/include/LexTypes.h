...
/*
    ...
    MEMBERS:
    * storageKind            - Kind of storage used on the current platform ("file", "registry" or "in-memory").
    * permissionFlag         - Permission flag in effect ("la-user", "la-system", "la-all-users" or "la-in-memory").
    * isCustomDataDirectory  - Whether the directory set using SetDataDirectory() is in effect (1) or not (0).
    * path                   - Location of the data store.
*/
typedef struct
{
    CHARTYPE storageKind[BUFFER_SIZE_256];
    CHARTYPE permissionFlag[BUFFER_SIZE_256];
    uint32_t isCustomDataDirectory;
    CHARTYPE path[BUFFER_SIZE_1024];
} DataStoreInfo;

/*
    STRUCT: LicenseLookupInfo

    MEMBERS:
    * key   - License key.
    * type  - Type of the license (e.g., "node-locked", "hosted-floating").
*/
typedef struct
{
    CHARTYPE key[BUFFER_SIZE_256];

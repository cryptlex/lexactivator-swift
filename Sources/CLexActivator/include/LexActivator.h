...
/*
    ...
    PURPOSE: Gets information about the data store used by LexActivator on the
    current platform, including the location where the data is stored.

    The path member is the fully resolved location of the data and does not
    include the name of the data file. If a custom directory has been set using
    the SetDataDirectory() function, that directory is returned.
    ...
    When the LA_IN_MEMORY flag is used, nothing is persisted, storageKind is
    "in-memory" and path is empty.

    This function must be called after calling the SetProductId() function.

    PARAMETERS:
    * dataStoreInfo - pointer to a DataStoreInfo struct that receives the
      information

    RETURN CODES: LA_OK, LA_E_PRODUCT_ID, LA_E_BUFFER_SIZE

    NOTE: The path is intended for inspection only. Do not delete, move or modify
    its contents. To release the activation and clear the activation data of your
    product, use the DeactivateLicense() function.
*/
LEXACTIVATOR_API int LA_CC GetDataStoreInfo(DataStoreInfo *dataStoreInfo);
...
/*
    ...
    PURPOSE: Retrieves the license information from the Cryptlex servers without
    ...
    This function must be called after SetProductData(). It does not
    require SetProductId() to be called first.

    PARAMETERS:
    * licenseKey - the license key to look up.
    * licenseLookupInfo - pointer to the struct that receives the license information.

    RETURN CODES: LA_OK, LA_E_PRODUCT_DATA, LA_E_LICENSE_KEY, LA_E_INET, LA_E_SERVER,
    LA_E_CLIENT, LA_E_RATE_LIMIT

    NOTE: This function neither validates the license nor stores anything on the
    device, and it has no effect on the activation state of the application. Use
    ActivateLicense() to activate the license.
*/
LEXACTIVATOR_API int LA_CC LookupLicense(CSTRTYPE licenseKey, LicenseLookupInfo *licenseLookupInfo);
...
/*
    ...
    LA_E_INET, LA_E_VM, LA_E_TIME, LA_E_TIME_MODIFIED, LA_E_ACTIVATION_LIMIT, LA_E_FREE_PLAN_ACTIVATION_LIMIT_REACHED, LA_E_SERVER, LA_E_CLIENT,
...
/*
    ...
    * filePath - path of the offline activation response file.
    ...
    LA_E_VM, LA_E_TIME, LA_E_TIME_MODIFIED, LA_E_FILE_PATH, LA_E_OFFLINE_RESPONSE_FILE_EXPIRED
*/
...
/*
    ...
    LA_E_VM, LA_E_TIME, LA_E_TIME_MODIFIED, LA_E_SERVER, LA_E_CLIENT, LA_E_COUNTRY, LA_E_IP, LA_E_RATE_LIMIT
*/
...
/*
    ...
    It clears the data locally without contacting the Cryptlex servers,
    so the activation is not released and continues to occupy an activation
    slot of the license.
...
/*
    The declarations below are added by this package, not by Cryptlex.
    These headers are maintained by hand — a version bump never rewrites
    them — so preserve this block when refreshing them from a native
    release, and update LexActivatorError.swift to match.

    The native library exposes two ways to read composite values: a struct-based
    form declared above, and a JSON form whose entry points are exported by the
    library but left out of the shipped header. This package uses the JSON form,
    as every other Cryptlex binding (.NET, Java, Python, Go, Rust, Dart) does:
    the struct form returns fixed-size arrays — UserLicense embeds 100 Metadata
    slots of 4 KB each — with no documented way to learn how many entries were
    written, so a caller has to guess at the count. The JSON form is
    self-delimiting.

    If a future native release drops these symbols the failure is a link error
    when Scripts/build-xcframework.sh link-checks the slices.

    Each function follows the same convention as the rest of the C API: it fills
    the caller's buffer and returns LA_E_BUFFER_SIZE if the buffer is too small.
*/

LEXACTIVATOR_API int LA_CC GetDataStoreInfoInternal(STRTYPE dataStoreInfo, uint32_t length);
...
LEXACTIVATOR_API int LA_CC LookupLicenseInternal(CSTRTYPE licenseKey, STRTYPE licenseLookupInfo, uint32_t length);
